...
alertaRouter.get("/logs", async (req, res) => {
  try {
    const alertas = (await collections.alertas
      ?.find({})
      .toArray()) as AlertaLog[];
...
alertaRouter.post("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send(
        "Não foi possível ativar o alarme devido a uma falha no sistema!\n" +
          error.message
      );
  }
  try {
    ...
    result
      ? res
          .status(201)
          .send(`Log de alerta criado com sucesso - id: ${result.insertedId}`)
      : res
          .status(500)
          .send(
            "O Alerta foi ativado, mas ocorreu uma falha ao gerar o log de alerta."
          );
  } catch (error:any) {
    console.error(error);
